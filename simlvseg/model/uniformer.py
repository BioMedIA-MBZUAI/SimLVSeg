...
class UniFormer(nn.Module):
    ...
    def forward_features(self, x):
        outs = []
        outs.append(x)
        outs.append(torch.zeros(x.shape[0], 0, x.shape[2]//2, x.shape[3]//2, device=x.device))
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            # print(x.device, skip.device, type(skip), skip.shape)
...
class UniFormerUNet(smp.Unet):
    def __init__(
        self,
        encoder_name: str = "uniformer_small",
        encoder_depth: int = 5,
        ...
        if encoder_name == 'uniformer_small':
            self.encoder = uniformer_small()
            encoder_out_channels = [3, 0, 64, 128, 320, 512]
        elif encoder_name == 'uniformer_base':
